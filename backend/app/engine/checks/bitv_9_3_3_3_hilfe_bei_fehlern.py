"""BITV 9.3.3.3 — Hilfe bei Fehlern.

WCAG 3.3.3 (AA): Wenn ein Formular Fehler automatisch erkennt, müssen die
Fehlermeldungen verständlich sein und einen Hinweis zur Korrektur geben
(z. B. erwartetes Format, Beispiel, Mindest-/Maximalangabe, konkrete
...
sind nicht dieser Prüfschritt (→ 9.3.3.1); native clientseitige Validierung
...
_TEST_ID = "BITV_9_3_3_3_HILFE_BEI_FEHLERN"
...
async def check_hilfe_bei_fehlern(ctx: CheckContext):
    """BITV 9.3.3.3 — Fehlermeldung ohne Hinweis, wie der Fehler zu korrigieren ist."""
    errors = []
    for form in ctx.soup.find_all("form"):
        ...
        for feld in form.find_all(["input", "select", "textarea"]):
            ...
            if not nachrichten:
                continue  # Fehlerzustand ohne Meldung → gehört zu 9.3.3.1
