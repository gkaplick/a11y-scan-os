"""WCAG 1.3.5 — Identifikation des Eingabezwecks (autocomplete).
...
- Mehrteilige autocomplete-Werte erlaubt (autocomplete="billing email").
- Weitere Eingabetypen einbezogen (date, number, month, week, time,
  datetime-local, range, color).
- Skip-Muster mit Wortgrenze statt Substring — "code" matcht "country_code"
  nicht mehr; "search"/"query"/"q" entfernt, damit das Suche-Mapping feuert.
"""
...
_AUTOCOMPLETE_MAPPING = {
    ...
    "search": ["suche", "search", "suchen", "durchsuchen", "finden", "find"],
...
async def check_autocomplete(ctx: CheckContext):
    """WCAG 1.3.5 — bekannte Felder ohne (passendes) autocomplete-Attribut."""
    errors = []
    root = ctx.soup
    for inp in root.find_all("input"):
        ...
        if not tokens:
            errors.append(finding(
                "WCAG_1_3_5_AUTOCOMPLETE",
                f"Input-Feld für '{expected}' benötigt autocomplete=\"{expected}\"",
                get_dom_path(inp),
            ))
            ...
            errors.append(finding(
                "WCAG_1_3_5_AUTOCOMPLETE",
                f"Input-Feld für '{expected}' hat autocomplete=\"{autocomplete}\", "
